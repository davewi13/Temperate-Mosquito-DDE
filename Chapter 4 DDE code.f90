MODULE define_DDEs
  ...
  DOUBLE PRECISION :: densa,densr,densh,UPS,SHP,MAXEGG
...
CONTAINS
  ...
  SUBROUTINE DDES(T,Y,Z,DY)
	...
	DEt = DEATHeggnow * E
    DLt = (densa*densr*(((1D0+COS(2D0*PI*(T-182.5D0-UPS)/365D0))/2D0)**SHP)*LAR/(20D0+densh*LAR)+DEATHlarnow)*LAR
    DPt = DEATHpupnow * PUP
    DAt = DEATHadunow * ADU
	...
	dSLdt =SL*(((densa*densr*(((1D0+COS(2D0*PI*(T-DL-182.5D0-UPS)/365D0))/2D0)**SHP)*Z(2,4)/(20D0+densh*Z(2,4)))+DEATHlarL)&
    * (1-dDLdt) - (densa*densr*(((1D0+COS(2D0*PI*(T-182.5D0-UPS)/365D0))/2D0)**SHP)*LAR / (20D0+densh*LAR)) - DEATHlarnow)
...
PROGRAM basicmodel
  ...
  INTEGER, PARAMETER :: NOUT=551D0
  DOUBLE PRECISION, PARAMETER :: T0=0D0,TFINAL=550D0
  ...
  IF (SOL%FLAG == 0) THEN
     ...
     WRITE(filename, '( "Fullmodel-medsurv-winterstart-butt4volume2years.dat" )' )
